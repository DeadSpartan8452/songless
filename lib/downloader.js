...
function findYtDlp() {
  ...
  // Le moteur fourni avec Songless prime sur les anciennes installations globales.
  candidates.unshift({ cmd: path.join(ROOT, 'tools', 'bin', 'yt-dlp.exe'), prefixArgs: [] });
...
async function listPlaylist(url, opts = {}) {
  const plafond = opts.maxItems === 5000 ? 5000 : 500;
  const limite = Math.max(1, Math.min(opts.limite || 100, plafond));
  ...
  const { stdout } = await runYtDlp(args, null, {timeoutMs:120000, acceptPartialJson:true});
...
async function downloadTrack(query, opts = {}) {
  ...
  if (opts.importSource) entry.importSource = require('../public/source-balance').sourceOf({importSource:opts.importSource});
