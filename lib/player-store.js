...
const duplicateComparison = require('./duplicate-comparison');
...
function cleanPartyHistory(history) {
  ...
  return history.slice(0, 200).map(entry => ({
    ...
    collective: entry.collective ? {
      result: entry.collective.result === 'won' ? 'won' : 'lost',
      sharedPoints: Math.max(0, Number(entry.collective.sharedPoints) || 0),
      targetPoints: Math.max(0, Number(entry.collective.targetPoints) || 0),
      bestStreak: Math.max(0, Number(entry.collective.bestStreak) || 0),
    } : null,
...
function normalize(data) {
  ...
  base.duplicateDecisions = cleanDuplicateDecisions(data.duplicateDecisions);
  base.updatedAt = data.updatedAt || base.updatedAt;
  return base;
}

function cleanDuplicateDecisions(items) {
  if (!Array.isArray(items)) return [];
  const decisions = [];
  const seen = new Set();
  for (const item of items.slice(-2000)) {
    const files = Array.isArray(item && item.files)
      ? [...new Set(item.files.map(file => cleanText(file, 300)).filter(Boolean))].sort()
      : [];
    const key = duplicateComparison.decisionKey(files);
    if (!key || seen.has(key) || (item && item.decision) !== 'distinct') continue;
    seen.add(key);
    decisions.push({
      key,
      files,
      decision: 'distinct',
      reason: cleanText(item && item.reason, 240),
...
function duplicateDecisions() {
  return JSON.parse(JSON.stringify(load().duplicateDecisions));
}

function recordDuplicateDecision(input) {
  const data = load();
  const cleaned = cleanDuplicateDecisions([{
    files: input && input.files,
    decision: input && input.decision,
    reason: input && input.reason,
  }]);
  if (!cleaned.length) throw new Error('La décision de doublon est invalide.');
  const decision = cleaned[0];
  data.duplicateDecisions = data.duplicateDecisions.filter(item => item.key !== decision.key);
  data.duplicateDecisions.push(decision);
  if (data.duplicateDecisions.length > 2000) data.duplicateDecisions.splice(0, data.duplicateDecisions.length - 2000);
  save();
  return JSON.parse(JSON.stringify(decision));
}

function deleteDuplicateDecision(key) {
  const data = load();
  const before = data.duplicateDecisions.length;
  data.duplicateDecisions = data.duplicateDecisions.filter(item => item.key !== String(key || ''));
  if (data.duplicateDecisions.length === before) return false;
  save();
  return true;
}
...
function cleanNamedList(items, prefix, { preserveUpdatedAt = false } = {}) {
  if (!Array.isArray(items)) return [];
  if (prefix === 'c') {
    return items.slice(0, 200).map(item => playlists.cleanPlaylist(item, { preserveUpdatedAt }));
  }
...
function playlistById(id) {
  const item = load().collections.find(entry => entry.id === String(id || ''));
  return item || null;
}

function allPlaylists() {
  return JSON.parse(JSON.stringify(load().collections));
}

function createPlaylist(input) {
  const data = load();
  const item = playlists.cleanPlaylist({ ...input, id: null }, { preserveUpdatedAt: false });
  playlists.log(item, { type: 'created', label: 'Playlist créée' });
  data.collections.unshift(item);
  if (data.collections.length > 200) data.collections.length = 200;
  save();
...
function updatePlaylist(id, patch) {
  const data = load();
  const index = data.collections.findIndex(entry => entry.id === String(id || ''));
  if (index < 0) return null;
  const current = data.collections[index];
  const allowed = patch && typeof patch === 'object' ? {
    nom: patch.nom !== undefined ? patch.nom : current.nom,
    description: patch.description !== undefined ? patch.description : current.description,
    collaborative: patch.collaborative !== undefined ? patch.collaborative : current.collaborative,
    quotaPerPlayer: patch.quotaPerPlayer !== undefined ? patch.quotaPerPlayer : current.quotaPerPlayer,
    reservePerPlayer: patch.reservePerPlayer !== undefined ? patch.reservePerPlayer : current.reservePerPlayer,
  ...
  const next = playlists.cleanPlaylist({ ...current, ...allowed }, { preserveUpdatedAt: false });
  next.createdAt = current.createdAt;
  next.activity = current.activity;
  playlists.log(next, { type: 'updated', label: 'Réglages de la playlist modifiés' });
  data.collections[index] = next;
  save();
  return JSON.parse(JSON.stringify(next));
}

function duplicatePlaylist(id, name) {
  const source = playlistById(id);
  if (!source) return null;
  const copy = playlists.cleanPlaylist({
    ...source,
    id: null,
    nom: name || `${source.nom} — copie`,
    status: 'draft',
    deadlineAt: null,
    contributions: [],
    activity: [],
  ...
  const data = load();
  playlists.log(copy, { type: 'duplicated', label: `Dupliquée depuis ${source.nom}` });
  data.collections.unshift(copy);
  save();
...
function deletePlaylist(id) {
  ...
  data.collections = data.collections.filter(entry => entry.id !== String(id || ''));
  if (data.collections.length === before) return false;
  save();
  return true;
}

function addPlaylistTrack(id, contribution, options) {
  const item = playlistById(id);
  if (!item) return null;
  const result = playlists.addTrack(item, contribution, options);
  save();
  return { playlist: JSON.parse(JSON.stringify(item)), ...result };
}

function removePlaylistTrack(id, trackId, actor) {
  const item = playlistById(id);
  if (!item) return null;
  const removed = playlists.removeTrack(item, trackId, actor);
  if (removed) save();
  return { removed, playlist: JSON.parse(JSON.stringify(item)) };
}

function removePlaylistContribution(id, trackId, profileId) {
  const item = playlistById(id);
  if (!item) return null;
  const removed = playlists.removeContribution(item, trackId, profileId);
  if (removed) save();
  return { removed, playlist: JSON.parse(JSON.stringify(item)) };
}

function reorderPlaylist(id, trackIds) {
  const item = playlistById(id);
  if (!item) return null;
  playlists.reorder(item, trackIds);
  save();
  return JSON.parse(JSON.stringify(item));
}

function markPlaylistPlayed(id) {
  const item = playlistById(id);
  if (!item) return null;
  item.sessionsPlayed = Math.max(0, Number(item.sessionsPlayed) || 0) + 1;
  item.lastPlayedAt = new Date().toISOString();
  item.updatedAt = item.lastPlayedAt;
  playlists.log(item, { type: 'played', label: 'Playlist utilisée pour une partie' });
  save();
  return JSON.parse(JSON.stringify(item));
}

function ratePlaylistTrack(id, input) {
  const item = playlistById(id);
  if (!item) return null;
  const score = playlists.rateTrack(item, input);
  save();
  return { score, playlist: JSON.parse(JSON.stringify(item)) };
}
...
function replaceAll(input) {
  ...
  for (const key of ['collections', 'challenges', 'partyHistory', 'blacklist', 'duplicateDecisions']) {
