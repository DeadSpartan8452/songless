...
function refreshBuzzer(party, now = Date.now()) {
  ...
  if (party.activeBuzzerProfileId && party.buzzerDeadline <= now) {
...
function command(party, hostToken, action, data = {}) {
  ...
  if (action === 'start-round') {
    ...
    party.playback = {
      // Deux secondes laissent au tunnel et aux téléphones le temps de charger
      // le flux avant le top commun.
      startedAt: now + 2_000,
  ...
  if (action === 'answer') {
    ...
    if (party.mode === 'buzzer') {
      ...
      if (player.correct) {
        ...
        player.answer = null;
        player.wrongAttempts++;
        player.buzzerBlockedUntil = Date.now() + BUZZER_PENALTY_MS;
      }
    }
...
function publicState(party, playerToken, hostToken) {
  ...
  return {
    ...
    status: party.status,
    round: party.round,
    playback: party.status === 'round' && party.playback ? { ...party.playback } : null,
    buzzer: {
      activeProfileId: party.activeBuzzerProfileId,
      answerSecondsRemaining: party.activeBuzzerProfileId
