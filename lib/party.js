...
function cleanPartySettings(input, mode = 'classic') {
  ...
  return {
    ...
    playlistId: String(source.playlistId || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 64),
  };
}
...
function setTrackIds(party, hostToken, values) {
  if (!isHost(party, hostToken)) throw new Error('Commande réservée à l’hôte.');
  if (party.status !== 'lobby') throw new Error('La playlist ne peut plus changer après le lancement.');
  party.trackIds = Array.isArray(values)
    ? [...new Set(values.map(value => String(value || '')).filter(Boolean))].slice(0, 5000)
    : [];
  party.totalRounds = party.infinite ? null : Math.min(
    Math.max(1, Number(party.totalRounds) || 10),
    Math.max(1, party.trackIds.length)
  );
  party.updatedAt = Date.now();
  return party.trackIds;
}
...
module.exports = {
  create, get, remove, join, isInvited, findPlayer, setTrackIds, command, playerAction, publicState,
