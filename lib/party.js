...
function command(party, hostToken, action, data = {}) {
  ...
  if (action === 'start-round') {
    ...
    party.autoNextAt = data.autoNext
      ? (party.playback ? party.playback.startedAt + highlightDuration * 1000 : now + 3_000)
      : null;
...
function playerAction(party, playerToken, action, data = {}) {
  ...
  if (action === 'vote-skip') {
    ...
    if (party.skipVotes.length >= voteThreshold(party)) party.roundDecision = 'skip';
    ...
    if (party.moreTimeVotes.length >= voteThreshold(party)) {
