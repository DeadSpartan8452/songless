...
const ACCESS_TOKEN_MIN_TTL_MS = 5 * 60 * 1000;
const ACCESS_TOKEN_MAX_TTL_MS = 12 * 60 * 60 * 1000;
...
function create({ mode = 'classic', totalRounds = 10, seed = '', settings = {}, trackIds = [] }) {
  ...
  const party = {
    ...
    inviteToken: token(),
    accessTokens: [],
    mode: modeRegistry.normalizeModeId(mode),
    infinite,
...
function issueAccessToken(party, hostToken, role, ttlMs = 2 * 60 * 60 * 1000) {
  ...
  const duration = Math.min(ACCESS_TOKEN_MAX_TTL_MS,
    Math.max(ACCESS_TOKEN_MIN_TTL_MS, Number(ttlMs) || 2 * 60 * 60 * 1000));
  const entry = {
...
function accessRole(party, accessToken) {
  ...
  const entry = party.accessTokens.find(candidate => candidate.tokenHash === hash);
...
function livingPlayers(party) {
  return party.players.filter(player => !player.isGhost && Number(player.lives) > 0);
}
...
function resolveFinalDuelRound(party) {
...
function resolveRoyaleReveal(party) {
  if (party.finalDuel && party.finalDuel.active) {
    resolveFinalDuelRound(party);
    return;
  }
  for (const player of party.players) {
    if (!player.isGhost && !player.found) {
      player.lives = Math.max(0, (player.lives !== undefined ? player.lives : 3) - 1);
      if (player.lives === 0) player.isGhost = true;
    }
  }
  const survivors = livingPlayers(party);
  if (survivors.length === 2) {
    activateFinalDuel(party);
  }
}
...
function command(party, hostToken, action, data = {}) {
  ...
  if (action === 'start-round') {
    ...
    if (data.teamId && !(party.teams || []).some(team => team.id === String(data.teamId))) {
...
function playerAction(party, playerToken, action, data = {}) {
  ...
  if (action === 'skip') {
    ...
    if (party.mode !== 'classic') throw new Error('Le vote de palier n’est actif qu’en réponses simultanées.');
...
module.exports = {
  ...
  issueAccessToken, revokeAccessToken, accessRole, TEAM_PRESETS, MYSTERY_MODIFIERS,
};
