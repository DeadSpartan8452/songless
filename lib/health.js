...
const trackQuality = require('./track-quality');
const mediaImages = require('./media-images');
const audioQuality = require('./audio-quality');
...
async function analyser(opts = {}) {
  ...
  const techniqueParFichier = new Map();
  ...
  for (let i = 0; i < fichiers.length; i++) {
    ...
    let taille = 0;
    let illisible = false;
    const pochetteAnnoncee = !!(meta && meta.hasCover);
    let pochetteValide = null;
    let pochetteInfo = null;
    let encodage = audioQuality.inspect();
    ...
    if (!duree || pochetteAnnoncee || deep) {
      try {
        const tags = await musicMetadata.parseFile(chemin, { duration: true });
        duree = duree || tags.format.duration || 0;
        encodage = audioQuality.inspect(tags.format);
        if (pochetteAnnoncee) {
          const infos = (tags.common.picture || [])
            .map(picture => mediaImages.embeddedPictureInfo(picture));
          pochetteInfo = infos.find(info => info.valid) || infos[0] || null;
          pochetteValide = Boolean(pochetteInfo && pochetteInfo.valid);
        }
      } catch (_) {
        illisible = !duree;
        if (pochetteAnnoncee) pochetteValide = false;
      }
    }

    const titre = (meta && meta.title) || T.fromFilename(nom).title || nom;

    for (const issue of encodage.issues) {
      ajouter({ ...issue, fichier: nom, titre });
    }
    ...
    if (!meta) {
      ...
      } else if (!['high', 'medium'].includes(meta.genreConfidence)) {
        ajouter({
          type: 'genre-incertain',
          gravite: 'cosmetique',
          fichier: nom,
          titre,
          detail: 'Genre présent, mais sa provenance ou sa confiance reste à confirmer.',
          action: 'Utilise le classement en lot ou le crayon pour valider ce genre.',
        });
      }
      if (!meta.year) {
        ajouter({
      ...
      } else if (!['high', 'medium'].includes(meta.yearConfidence)) {
        ajouter({
          type: 'annee-incertaine',
          gravite: 'cosmetique',
          fichier: nom,
          titre,
          detail: 'Année présente, mais sa provenance ou sa confiance reste à confirmer.',
          action: 'Ouvre le crayon pour préciser la source et le niveau de confiance.',
        });
      }
      if (Object.hasOwn(meta, 'hasCover') && meta.hasCover === false) {
        ajouter({
          type: 'sans-pochette',
          gravite: 'cosmetique',
          fichier: nom,
          titre,
          detail: 'Aucune pochette intégrée connue : le fallback local sera utilisé.',
          action: 'Facultatif : ajoute une pochette au fichier audio, sans toucher au favicon.',
        });
      } else if (pochetteAnnoncee && pochetteValide === false) {
        ajouter({
          type: 'pochette-invalide',
          gravite: 'genant',
          fichier: nom,
          titre,
          detail: 'La fiche annonce une pochette, mais ses octets ne correspondent pas à une image valide.',
          action: 'Remplace la pochette intégrée ; le jeu utilisera son fallback local entre-temps.',
        });
      } else if (pochetteAnnoncee && pochetteInfo && pochetteInfo.valid
          && (!pochetteInfo.width || !pochetteInfo.height)) {
        ajouter({
          type: 'pochette-dimensions-inconnues',
          gravite: 'cosmetique',
          fichier: nom,
          titre,
          detail: 'La pochette est une image valide, mais ses dimensions ne sont pas lisibles.',
          action: 'Réintègre une image PNG, JPEG, GIF ou WebP complète.',
        });
      } else if (pochetteAnnoncee && pochetteInfo && pochetteInfo.valid
          && Math.min(pochetteInfo.width, pochetteInfo.height) < 256) {
        ajouter({
          type: 'pochette-petite',
          gravite: 'cosmetique',
          fichier: nom,
          titre,
          detail: `${pochetteInfo.width} × ${pochetteInfo.height} px : la pochette risque d’être floue en grand.`,
          action: 'Facultatif : remplace-la par une image carrée d’au moins 300 × 300 px.',
        });
      } else if (pochetteAnnoncee && pochetteInfo && pochetteInfo.valid
          && (Math.max(pochetteInfo.width, pochetteInfo.height) > 5000
            || pochetteInfo.bytes > 10 * 1024 * 1024)) {
        ajouter({
          type: 'pochette-surdimensionnee',
          gravite: 'cosmetique',
          fichier: nom,
          titre,
          detail: `${pochetteInfo.width} × ${pochetteInfo.height} px, ${Math.round(pochetteInfo.bytes / 1024 / 1024)} Mo : inutilement lourde pour le jeu.`,
          action: 'Facultatif : réintègre une copie raisonnable sans modifier le fichier source de l’image.',
        });
      }
      if (meta.needsReview) {
        ajouter({
  ...
  for (let i = 0; i < fiches.length; i++) {
    ...
    for (let j = i + 1; j < fiches.length; j++) {
      ...
      risqueDoublon.add(fiches[i].fileName);
      risqueDoublon.add(fiches[j].fileName);
      ajouter({
        type: 'doublon',
        gravite: 'genant',
        ...
        action: 'Compare les deux fiches. Toute mise à l’écart exige une décision manuelle.',
      });
    }
  }
  ...
  const problemesParFichier = new Map();
  for (const probleme of problemes) {
    if (!probleme.fichier) continue;
    const liste = problemesParFichier.get(probleme.fichier) || [];
    liste.push(probleme);
    problemesParFichier.set(probleme.fichier, liste);
  }
  const qualiteMorceaux = fichiers.map((fichier) => {
    const meta = metaTracks[fichier] || {};
    const technique = techniqueParFichier.get(fichier) || {
      encoding: audioQuality.inspect(), cover: null,
    };
    return {
      fichier,
      technical: technique,
      ...trackQuality.assess({
        issues: problemesParFichier.get(fichier) || [],
        duplicateRisk: risqueDoublon.has(fichier),
        coverKnown: Object.hasOwn(meta, 'hasCover'),
        coverDimensionsKnown: meta.hasCover === false || Boolean(
          technique.cover && technique.cover.width && technique.cover.height
        ),
        encodingKnown: technique.encoding.known,
        deep,
      }),
    };
  });
  const qualite = {
    ready: qualiteMorceaux.filter(item => item.status === 'ready').length,
    review: qualiteMorceaux.filter(item => item.status === 'review').length,
    problematic: qualiteMorceaux.filter(item => item.status === 'problematic').length,
    averageScore: qualiteMorceaux.length
      ? Math.round(qualiteMorceaux.reduce((total, item) => total + item.score, 0)
        / qualiteMorceaux.length)
      : 0,
    encodingMeasured: qualiteMorceaux.filter(item => item.technical.encoding.known).length,
    coversDeclared: qualiteMorceaux.filter(item => item.technical.cover).length,
    coverDimensionsMeasured: qualiteMorceaux.filter(item => (
      item.technical.cover && item.technical.cover.width && item.technical.cover.height
    )).length,
  };
