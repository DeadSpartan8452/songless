...
const YEAR_SOURCES = new Set([
  'manual', 'musicbrainz', 'tag', 'youtube', 'youtube-upload', 'filename', 'unknown',
]);
const GENRE_SOURCES = new Set([
  'manual', 'musicbrainz', 'tag', 'youtube', 'filename', 'unknown',
]);
...
function confidenceFromSource(source, value, field) {
  ...
  if (value === 'musicbrainz' || value === 'tag' || value === 'youtube') return 'medium';
  if (value === 'youtube-upload' || value === 'filename') return 'low';
  return 'unknown';
}
...
function reviewStatus(input = {}) {
  ...
  const unofficialVariant = source.unofficialVariant === true;
  const yearMissing = !unofficialVariant && !classification.year;
  const artistMissing = !unofficialVariant && (!artist || artist === 'Artiste inconnu');
  const artistUncertain = !unofficialVariant && !artistMissing
    && !['high', 'medium'].includes(clean(source.artistConfidence, 16));
  const genreUncertain = !genreMissing
    && !['high', 'medium'].includes(classification.genreConfidence);
  const yearUncertain = !unofficialVariant && !yearMissing
    && !['high', 'medium'].includes(classification.yearConfidence);

  return {
    any: genreMissing || genreUncertain || yearMissing || yearUncertain
      || artistMissing || artistUncertain,
    genre: genreMissing || genreUncertain,
    year: yearMissing || yearUncertain,
    artist: artistMissing || artistUncertain,
...
function isTrusted(input, field) {
  const source = input && typeof input === 'object' ? input : {};
  const classification = readClassification(source);
  if (field === 'genre') {
    const genre = clean(source.genre, 80);
    return Boolean(genre && genre !== 'Autre'
      && ['high', 'medium'].includes(classification.genreConfidence));
  }
  if (field === 'year') {
    return Boolean(classification.year
      && ['high', 'medium'].includes(classification.yearConfidence));
  }
  return false;
}
