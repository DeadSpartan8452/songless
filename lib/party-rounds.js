...
async function startNextPartyRound(party, dependencies) {
  ...
  if (party.mode === 'intruder') {
    const challenge = await dependencies.buildIntruderChallenge(party);
    if (!challenge) {
      throw new Error('Les métadonnées ne permettent pas de créer un Intrus certain.');
    }
    dependencies.command(party, party.hostToken, 'start-round', {
      round: party.round + 1,
      trackId: challenge.options[0].id,
      answer: null,
      intruderChallenge: challenge,
    });
    return;
  }
