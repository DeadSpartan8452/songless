...
const TARGET_TYPES = Object.freeze([
  'track', 'library', 'artist', 'genre', 'theme', 'year', 'decade',
]);
...
function normalizeRule(input, previous = null, nowValue = new Date()) {
  ...
  const rawIds = previous && previous.targetType === 'library'
    ? previous.trackIds : source.trackIds;
  const trackIds = targetType === 'library' && Array.isArray(rawIds)
    ? [...new Set(rawIds.map(value => text(value, 2048)).filter(Boolean))] : [];
  if (targetType === 'library' && (!trackIds.length || trackIds.length > 50000)) {
    throw new Error('La bibliothèque à écarter doit contenir entre 1 et 50 000 morceaux.');
  }
...
function matches(rule, track, mode, nowValue = new Date()) {
  ...
  switch (rule.targetType) {
    case 'library': return Array.isArray(rule.trackIds) && rule.trackIds.includes(String(track && track.id));
