...
const clamavDatabase = require('./clamav-database');

function runtimeEnvironment() {
  const env = {...process.env};
  if (process.env.SONGLESS_NATIVE_LIB_DIR) {
    env.LD_LIBRARY_PATH = process.env.SONGLESS_NATIVE_LIB_DIR;
  }
  return env;
}
...
function status() {
  ...
  if (!engine) return { available: false, id: null, name: null };
  if (engine.id === 'clamav' && process.env.SONGLESS_CLAMAV_DB) {
    const database = clamavDatabase.status();
    return {
  ...
  return { available: true, id: engine.id, name: engine.name };
}

function scanArguments(engine, file, options = {}) {
  ...
  if (engine.id === 'clamav') {
    const args = [];
    if (process.env.SONGLESS_CLAMAV_CERTS) {
      args.push(`--cvdcertsdir=${process.env.SONGLESS_CLAMAV_CERTS}`);
    }
    if (process.env.SONGLESS_CLAMAV_DB) {
      args.push(`--database=${process.env.SONGLESS_CLAMAV_DB}`);
    }
    if (options.recursive) args.push('--recursive=yes');
    return [...args, '--no-summary', '--', file];
  }
...
function scan(file, { timeout = 10 * 60 * 1000 } = {}) {
  ...
  if (engine.id === 'clamav' && process.env.SONGLESS_CLAMAV_DB
      && !clamavDatabase.status().ready) {
    return Promise.reject(new Error('Bases antivirus Android absentes : installe-les avant l’import.'));
  }
  const resolved = path.resolve(file);
  if (!fs.existsSync(resolved)) return Promise.reject(new Error('Fichier à analyser introuvable.'));
  const recursive = fs.statSync(resolved).isDirectory();

  return new Promise((resolve, reject) => {
    const child = spawn(engine.command, scanArguments(engine, resolved, {recursive}), {
      windowsHide: true,
      shell: false,
      env: runtimeEnvironment(),
    });
...
module.exports = {
  ...
  runtimeEnvironment, scanArguments, status,
};
