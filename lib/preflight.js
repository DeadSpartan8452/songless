...
async function run(options) {
  ...
  const dependenciesOk = options.dependenciesOk !== false;
  const missingDependencies = Array.isArray(options.missingDependencies)
    ? options.missingDependencies.filter(Boolean) : [];
  const missingDetail = missingDependencies.length
    ? ` : ${missingDependencies.join(', ')}` : '';
  checks.push(result('node', 'Node.js et dépendances', major >= 18 && dependenciesOk ? 'green' : 'blocking',
    `Node.js ${process.versions.node} · ${dependenciesOk ? 'dépendances présentes' : `dépendances incomplètes${missingDetail}`}`,
    dependenciesOk ? '' : options.mobileHost
      ? 'Réinstalle l’APK Songless complet.'
      : 'Répare Songless depuis son lanceur.'));
  ...
  const antivirus = options.antivirus || { available: false, name: null };
  checks.push(antivirus.available
    ? result('antivirus', 'Protection des imports', 'green', `${antivirus.name} est disponible.`)
    : result('antivirus', 'Protection des imports', 'blocking', 'Aucun moteur antivirus compatible.',
      process.platform === 'win32' ? 'Réactive Microsoft Defender avant tout import.'
        : 'Installe ClamAV et ses signatures avant tout import.'));
...
function checkDependencies(root, dependencies) {
  const missing = Object.keys(dependencies || {}).filter(name => {
    try {
      ...
      if (!packageFile.startsWith(`${modulesRoot}${path.sep}`)) return true;
      try {
