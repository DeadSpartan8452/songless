...
function cleanTrack(track) {
  ...
  const genreTrusted = ['high', 'medium'].includes(track && track.genreConfidence);
  const yearTrusted = ['high', 'medium'].includes(track && track.yearConfidence);
  return {
    id: text(track && track.id, 300),
    title: text(track && track.title, 200),
    artist: text(track && track.artist, 160),
    year: yearTrusted && Number.isInteger(year) && year >= 1900 && year <= 2100
      ? year : null,
    genre: genreTrusted ? text(track && track.genre, 100) : '',
    theme: genreTrusted ? text(track && (track.theme || track.genreDetail), 100) : '',
  };
}
