...
function partyHistoryEntry(party) {
  ...
  const winnerProfileId = party.winnerProfileId
    ? String(party.winnerProfileId) : null;
  const winner = party.mode === 'cooperation'
    ? null
    : winnerProfileId
  ...
  return {
    ...
    collective: party.mode === 'cooperation' && party.cooperation ? {
      result: party.cooperation.result,
      sharedPoints: Number(party.cooperation.sharedPoints) || 0,
      targetPoints: Number(party.cooperation.targetPoints) || 0,
      bestStreak: Number(party.cooperation.bestStreak) || 0,
    } : null,
