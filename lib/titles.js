...
// Les services de téléchargement placent parfois la variante avant le titre
// ("Nightcore Promet pas la lune") et parfois après. Pour les suggestions,
// ces deux libellés désignent le même morceau. On ne réordonne jamais les
// autres mots : deux vrais titres dont l'ordre diffère restent donc distincts.
const VARIANT_PHRASES = [
  ['bass boosted', 'bassboosted'],
  ['sped up', 'spedup'],
  ['speed up', 'spedup'],
  ['slowed reverb', 'slowedreverb'],
  ['slowed and reverb', 'slowedreverb'],
  ['slowed', 'slowed'],
  ['reverb', 'reverb'],
  ['nightcore', 'nightcore'],
  ['remix', 'remix'],
...
/** Clé qui stabilise la position des marqueurs de variante connus. */
function suggestionVariantKey(str) {
  let normalized = ` ${norm(str)} `;
  const variants = [];
  for (const [phrase, key] of VARIANT_PHRASES) {
  ...
  if (!variants.length) return tightKey(str);
  return `${main}::${[...new Set(variants)].sort().join('+')}`;
}

/** Le texte est-il lisible/tapable avec un clavier latin ? */
function isReadableLatin(str) {
  const letters = Array.from(String(str || '')).filter(unicode.isLetter);
  if (letters.length === 0) return true;
  const latin = letters.filter(unicode.isLatinLetter).length;
