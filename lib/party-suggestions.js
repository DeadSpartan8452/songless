...
function suggestions({ query, answerMode, fileNames, metadata, limit = 16 }) {
  ...
  for (const fileName of fileNames || []) {
    ...
    const valueKey = answerMode === 'titre'
      ? `${T.tightKey(artist)}::${T.suggestionVariantKey(title)}`
      : T.tightKey(value);
  ...
  return results.slice(0, Math.min(24, Math.max(0, Number(limit) || 16)))
