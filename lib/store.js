...
const META_FILE = process.env.SONGLESS_METADATA_FILE
  ? path.resolve(process.env.SONGLESS_METADATA_FILE)
  : path.join(ROOT, 'metadata.json');
const BACKUP_DIR = process.env.SONGLESS_METADATA_BACKUP_DIR
  ? path.resolve(process.env.SONGLESS_METADATA_BACKUP_DIR)
  : path.join(path.dirname(META_FILE), 'metadata-backups');
