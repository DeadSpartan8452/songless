...
function roundPoints(party, seconds) {
  const multiplier = Math.max(0, Number(
    party.roundModifier && party.roundModifier.multiplier
  ) || 1);
  return Math.round(pointsFor(party.settings && party.settings.points, seconds) * multiplier);
}
...
function resolveCorrect(party, player) {
  ...
  const points = roundPoints(party, bid && bid.seconds);
  auction.phase = 'resolved';
  auction.activeProfileId = null;
...
function publicState(party, viewer) {
  ...
  return {
    ...
    options: BID_OPTIONS.map(seconds => ({
      seconds,
      points: roundPoints(party, seconds),
    })),
    bids,
...
module.exports = {
  BID_OPTIONS, BID_SECONDS, canAnswer, finalize, pointsFor, publicState, roundPoints,
